...
func TestGitLabExecutorExecuteUsesRequestedModelToSelectOpenAIGateway(t *testing.T) {
	...
	// The codex executor in this tree stamps its own codex-tui User-Agent after
	// custom headers are applied, so only require that a User-Agent was sent.
	if gotUserAgent == "" {
		t.Fatal("expected a User-Agent header on the OpenAI gateway request")
	}
