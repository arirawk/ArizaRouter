...
func (e *ClineExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		logClineUpstreamError(ctx, baseModel, url, httpResp.StatusCode, httpResp.Header.Get("Content-Type"), b)
...
// logClineUpstreamError records a non-2xx upstream response with a trimmed body summary.
func logClineUpstreamError(ctx context.Context, model, url string, statusCode int, contentType string, body []byte) {
	helps.LogWithRequestID(ctx).Warnf("cline model=%s upstream error: url=%s status=%d message=%s", model, url, statusCode, helps.SummarizeErrorBody(contentType, body))
}
