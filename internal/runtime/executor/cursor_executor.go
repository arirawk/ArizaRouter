...
// CountTokens estimates token count locally using tiktoken. Cursor has no
// token-counting endpoint, so the inbound payload is translated to the OpenAI
// chat schema and counted with the model's tokenizer.
func (e *CursorExecutor) CountTokens(ctx context.Context, _ *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if from.String() != "" && from.String() != "openai" {
		usageJSON = sdktranslator.TranslateTokenCount(ctx, to, from, count, usageJSON)
	}
