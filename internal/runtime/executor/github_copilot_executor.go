...
func (e *GitHubCopilotExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body = helps.ApplyPayloadConfigWithRoot(e.cfg, req.Model, to.String(), "", body, originalTranslated, requestedModel, helps.PayloadRequestPath(opts))
...
func (e *GitHubCopilotExecutor) logUpstreamError(ctx context.Context, auth *cliproxyauth.Auth, requestedModel string, requestBody []byte, url string, statusCode int, contentType string, responseBody []byte) {
	...
	helps.LogWithRequestID(ctx).Warnf("%s model=%s upstream error: url=%s status=%d message=%s", logProvider, logModel, url, statusCode, helps.SummarizeErrorBody(contentType, responseBody))
}
...
// useGitHubCopilotResponsesEndpoint reports whether the request should go to
// /responses instead of /chat/completions. This core's registry.ModelInfo has
// no per-model endpoint list, so the decision is based on the source format
// and on codex-family model names only.
