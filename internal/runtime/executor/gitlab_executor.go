...
func openAIContentText(content gjson.Result) string {
	...
	collectGitLabOpenAIContent(content, &segments)
	return strings.TrimSpace(strings.Join(segments, "\n"))
}

// collectGitLabOpenAIContent flattens an OpenAI message content value (a plain
// string or an array of typed parts) into its text segments.
func collectGitLabOpenAIContent(content gjson.Result, segments *[]string) {
	switch {
		...
		content.ForEach(func(_, part gjson.Result) bool {
			...
			for _, key := range []string{"text", "input_text", "content"} {
				...
				if value.Type == gjson.String {
					...
					collectGitLabOpenAIContent(value, segments)
				}
				break
			}
			return true
	...
	case content.IsObject():
		for _, key := range []string{"text", "content"} {
			if value := content.Get(key); value.Exists() {
				collectGitLabOpenAIContent(value, segments)
				return
			}
		}
...
func inferGitLabProviderFromModel(model string) string {
	return registry.InferGitLabProviderFromModel(model)
}
...
func gitLabResolvedModel(auth *cliproxyauth.Auth, requested string) string {
	...
	if requested != "" && !strings.EqualFold(requested, "gitlab-duo") {
		if mapped, ok := registry.GitLabModelAliases[strings.ToLower(requested)]; ok && strings.TrimSpace(mapped) != "" {
			return mapped
		}
		return requested
	}
...
// GitLabModelsFromAuth returns the built-in GitLab Duo catalog plus any models
// discovered from the account's direct-access metadata.
func GitLabModelsFromAuth(auth *cliproxyauth.Auth) []*registry.ModelInfo {
	builtin := registry.GetGitLabModels()
	models := make([]*registry.ModelInfo, 0, len(builtin)+4)
	seen := make(map[string]struct{}, len(builtin)+4)
	...
	for _, model := range gitlab.ExtractDiscoveredModels(auth.Metadata) {
		...
		models = append(models, &registry.ModelInfo{
			...
			Description: strings.TrimSpace(model.ModelProvider),
			UserDefined: true,
		})
	}
	return models
