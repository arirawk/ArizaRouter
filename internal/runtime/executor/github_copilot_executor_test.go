...
func TestGitHubCopilotLogUpstreamErrorWarnsWithAuthAndModels(t *testing.T) {
...
func TestNewGitHubCopilotExecutorIdentifier(t *testing.T) {
	t.Parallel()
	e := NewGitHubCopilotExecutor(&config.Config{})
	if got := e.Identifier(); got != "github-copilot" {
		t.Fatalf("Identifier() = %q, want github-copilot", got)
	}
	if e.cache == nil {
		t.Fatal("expected token cache to be initialised")
	}
}
