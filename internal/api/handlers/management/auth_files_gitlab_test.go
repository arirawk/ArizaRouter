...
func TestRequestGitLabPATToken_SavesAuthRecord(t *testing.T) {
	...
	if state == "" {
		t.Fatalf("expected state in response, got %#v", resp["state"])
	}
	t.Cleanup(func() { CancelOAuthSession(state) })
	provider, status, _, _, completed, ok := GetOAuthSessionDetails(state)
	if !ok || provider != "gitlab" {
		t.Fatalf("expected gitlab session for %s, got provider=%q ok=%v", state, provider, ok)
	}
	if !completed || status != "" {
		t.Fatalf("session completed=%v status=%q, want completed with no error", completed, status)
	}
...
func TestRequestGitLabPATToken_AcceptsQueryToken(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	gin.SetMode(gin.TestMode)
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/api/v4/user":
			_ = json.NewEncoder(w).Encode(map[string]any{"id": 1, "username": "query-user"})
		default:
			http.NotFound(w, r)
		}
	}))
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v0/management/gitlab-auth-url?token=glpat-query&base_url="+upstream.URL, nil)
	...
	for _, item := range store.items {
		if got := item.Metadata["personal_access_token"]; got != "glpat-query" {
			t.Fatalf("personal_access_token = %#v, want glpat-query", got)
		}
		if item.FileName != "gitlab-query-user-pat.json" {
			t.Fatalf("file name = %q, want gitlab-query-user-pat.json", item.FileName)
		}
	}
}

func TestRequestGitLabPATToken_RejectsMissingToken(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	gin.SetMode(gin.TestMode)
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v0/management/gitlab-auth-url", nil)
