...
func (s *Server) setupRoutes() {
	...
	s.engine.GET("/cline/callback", func(c *gin.Context) {
		...
		if state == "" {
			// Cline does not always echo the state parameter back; attribute the
			// callback to the single pending Cline session when there is exactly one.
			if pending, ok := managementHandlers.PendingOAuthSessionState("cline"); ok {
				state = pending
			}
		}
		if state != "" {
